...
// 获取可以调休的时间（分钟）GET /api/v2/Apply/GetBreakOff
export function GetBreakOff(token) {
  console.log(token)
  return request({
    url: 'apix/v2/Apply/GetBreakOff',
    method: 'GET',
    params: { token }
  })
}

// /api/v2/Apply/GetTimeInterval 获取申请中各种类型的时间间隔
export function GetTimeInterval(token, start, end, type) {
  console.log(token, start, end, type)
  return request({
    url: 'apix/v2/Apply/GetTimeInterval',
    method: 'GET',
    params: { token, start, end, type }
  })
}
